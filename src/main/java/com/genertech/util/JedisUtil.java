...
public class JedisUtil {
	...
	static{
		...
		//redisIp = PropertiesProvider.getProperty("redis.ip");
		//redisPort = 6379;
	}
	...
	public  static Jedis getJedis() {
		...
		do{
			try{
				jedis = getPool(redisIp, redisPort).getResource();
			} catch (Exception e) {
				log.error("get redis master1 failed!", e);
				removeJedisCache(jedis, redisIp, redisPort);
			}
			count++;
	...
	public    static void removeJedisCache(Jedis jedis, String ip, int port) {
		String key = ip+":" +port;
		if(maps.get(key)!=null) {
